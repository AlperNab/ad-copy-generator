...
**Domain:** `Marketing / Ads` · **Suite:** `E-commerce Growth Suite` · **Accent:** `#a3e635`
...
**Ad Copy Generator** is a standalone, browser-based AI workflow app for **Marketing / Ads**. It turns structured inputs, uploaded files, and project-specific settings into reviewable outputs using a deterministic local engine plus optional local/cloud LLM enhancement.
...
| `audience`<br>audience | select | Yes | general | Affects input: audience. |
...
| `forbidden_words`<br>forbidden words | textarea | No | — | Affects customization: forbidden words. |
| `platform`<br>platform | select | No | Meta Ads | Affects customization: platform. |
| `audience_persona`<br>audience persona | select | No | general | Affects customization: audience persona. |
| `offer_type`<br>offer type | text | No | — | Affects customization: offer type. |
| `budget_level`<br>budget level | select | No | low | Affects customization: budget level. |
| `claim_strictness`<br>claim strictness | slider | No | 50 | Affects customization: claim strictness. |
...
| `cta_style`<br>CTA style | select | No | soft | Affects customization: CTA style. |
...
- **audience persona**: general, executive, technical, legal, medical, student, customer
...
- **CTA style**: soft, direct, urgent, educational, premium
...
| `AI_SUITE_PORT` | Port for this project GUI, default `9100`. |
...
- platform-specific paid ad variants
...
```bash
curl -X POST http://127.0.0.1:9100/api/run \
...
```bash
AI_SUITE_HOST=0.0.0.0 AI_SUITE_PORT=9100 uvicorn app.main:app --host 0.0.0.0 --port 9100
```
...
- `plugins/ad-copy-generator.json` for plugin metadata.
